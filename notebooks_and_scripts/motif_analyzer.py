...
def motif_analyzer(motif, outfile_name):
	...
	count = Counter()
	data.apply(
		lambda x: count.update(convert_to_int(x['identified_motif_locations'])), 
		axis = 1
		)
...
if __name__ == '__main__':
	motif_analyzer(
		motif=r'[cC]-*[gG]', 
		outfile_name='counter_file_cpg.csv'
		)
	motif_analyzer(
		motif=zap_n_motif_constructor(4), 
		outfile_name='counter_file_zap_4_motif.csv'
		)
	motif_analyzer(
		motif=zap_n_motif_constructor(5), 
		outfile_name='counter_file_zap_5_motif.csv'
		)
	motif_analyzer(
		motif=zap_n_motif_constructor(6), 
		outfile_name='counter_file_zap_6_motif.csv'
		)
	motif_analyzer(
		motif=zap_n_motif_constructor(7), 
		outfile_name='counter_file_zap_7_motif.csv'
		)
	motif_analyzer(
		motif=zap_n_motif_constructor(8), 
		outfile_name='counter_file_zap_8_motif.csv'
		)